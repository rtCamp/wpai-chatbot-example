'use client';

import { useTransition, useEffect, useState } from 'react';
import { usePathname, useSearchParams } from 'next/navigation';
import { Button } from '@wpai-chatbot/chat/components/ui/button';
import { getTrackUID } from '@wpai-chatbot/chat/lib/utils';

import { signInLogto } from './actions/signIn';

export function SignInButton({
	onButtonClick,
}: {
	onButtonClick: (clicked: boolean) => void;
}) {
	const [isPending, startTransition] = useTransition();
	const pathname = usePathname();
	const [isInIframe, setIsInIframe] = useState(false);
	const searchParams = useSearchParams();

	// Detect if we're running in an iframe on component mount
	useEffect(() => {
		setIsInIframe(window.self !== window.top);
	}, []);

	useEffect(() => {
		const autoLogin = searchParams.get('autoLogin');
		if (!isInIframe && autoLogin === 'true') {
			const formData = new FormData();
			formData.append('returnUrl', pathname);
			formData.append('provider', 'google');
			formData.append('track_uid', getTrackUID() ?? '');

			startTransition(() => {
				signInLogto(formData);
			});
		}
	}, []); // eslint-disable-line react-hooks/exhaustive-deps

	const handleSignIn = (provider: string) => {
		onButtonClick(true);
		// If we're in an iframe, send a message to the parent window
		if (isInIframe) {
			const currentUrl = new URL(window.location.origin + pathname);
			currentUrl.searchParams.set('track_uid', getTrackUID() ?? '');
			currentUrl.searchParams.set('autoLogin', 'true');
			window.parent.postMessage(
				{
					type: 'WPAI_CHATBOT_AUTH_REQUIRED',
					returnUrl: currentUrl.toString(),
				},
				'*',
			);
			return;
		}

		// Standard flow for non-iframe context
		const formData = new FormData();
		formData.append('returnUrl', pathname);
		formData.append('provider', provider);
		formData.append('track_uid', getTrackUID() ?? '');

		startTransition(() => {
			signInLogto(formData);
		});
	};

	return (
		<>
			<Button
				onClick={() => {
					handleSignIn('google');
				}}
				disabled={isPending}
				className="w-full bg-primary text-primary-foreground rounded-md py-2"
			>
				{isPending ? 'Signing in...' : 'Sign in with Google'}
				<svg
					xmlns="http://www.w3.org/2000/svg"
					width="24"
					height="24"
					viewBox="0 0 24 24"
					fill="none"
				>
					<path
						fillRule="evenodd"
						clipRule="evenodd"
						d="M23.52 12.2729C23.52 11.422 23.4436 10.6038 23.3018 9.81836H12V14.4602H18.4582C18.18 15.9602 17.3345 17.2311 16.0636 18.082V21.0929H19.9418C22.2109 19.0038 23.52 15.9275 23.52 12.2729Z"
						fill="#4285F4"
					/>
					<path
						fillRule="evenodd"
						clipRule="evenodd"
						d="M12 23.9999C15.24 23.9999 17.9564 22.9254 19.9418 21.0926L16.0636 18.0817C14.9891 18.8017 13.6145 19.2272 12 19.2272C8.87455 19.2272 6.22909 17.1163 5.28546 14.2799H1.27637V17.389C3.25091 21.3108 7.30909 23.9999 12 23.9999Z"
						fill="#34A853"
					/>
					<path
						fillRule="evenodd"
						clipRule="evenodd"
						d="M5.28545 14.2798C5.04545 13.5598 4.90909 12.7907 4.90909 11.9998C4.90909 11.2089 5.04545 10.4398 5.28545 9.71981V6.61072H1.27636C0.463636 8.23072 0 10.0634 0 11.9998C0 13.9362 0.463636 15.7689 1.27636 17.3889L5.28545 14.2798Z"
						fill="#FBBC05"
					/>
					<path
						fillRule="evenodd"
						clipRule="evenodd"
						d="M12 4.77273C13.7618 4.77273 15.3436 5.37818 16.5873 6.56727L20.0291 3.12545C17.9509 1.18909 15.2345 0 12 0C7.30909 0 3.25091 2.68909 1.27637 6.61091L5.28546 9.72C6.22909 6.88364 8.87455 4.77273 12 4.77273Z"
						fill="#EA4335"
					/>
				</svg>
			</Button>
			{/* Uncomment for Microsoft */}
			{/* <Button
        onClick={() => { handleSignIn("msft") }}
        disabled={isPending}
        className="w-full bg-primary text-primary-foreground rounded-md py-2"
      >
        {isPending ? "Signing in..." : "Sign in with Microsoft"}
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 23 23"><path d="M0 0h23v23H0z" /><path fill="#f35325" d="M1 1h10v10H1z" /><path fill="#81bc06" d="M12 1h10v10H12z" /><path fill="#05a6f0" d="M1 12h10v10H1z" /><path fill="#ffba08" d="M12 12h10v10H12z" /></svg>
      </Button> */}
		</>
	);
}
